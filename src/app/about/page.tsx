...
export default function About() {
  ...
  useEffect(() => {
    // Force a new key on each mount to ensure fresh animation
    setKey(Date.now());
    console.log('About page loaded, setting slide-in animation');
  }, []);

  const handleBackClick = (e: React.MouseEvent) => {
    e.preventDefault();
    console.log('Back clicked, starting slide-out animation');
  ...
  console.log('About page state:', { isNavigating, key });

  return (
    <>
      {/* Fixed Back link in top right corner - same position as About, no animation */}
      <div key={`back-${key}`} className="fixed top-8 right-8 z-50">
        <Link href="/" onClick={handleBackClick} className="text-blue-700 dark:text-blue-400 hover:underline text-lg">
          Back
        </Link>
      </div>

      <main key={`main-${key}`} className={`max-w-2xl mx-auto px-8 py-20 ${
        isNavigating ? 'animate-slide-out-right' : 'animate-slide-in-right'
